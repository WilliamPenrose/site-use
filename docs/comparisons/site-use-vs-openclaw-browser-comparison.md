...
| 无人值守运行 | 新 profile 指纹可能触发"新设备"风控 |
| 28+ 项反检测诊断 | 不支持 OOPIF |
| CDP 直连（puppeteer，低延迟） | 仅本地，无远程控制 |
| screenX/Y 坐标修复 | 未处理弹窗 |
| WebRTC 防泄漏 | 单路径元素定位（无 fallback） |
...
|---|---|
| 零自动化标记（真实浏览器） | 无行为拟人化 |
| 无 Runtime.Enable 泄漏 | 需用户手动附加标签页 |
| 完全真实的指纹 + 登录态 | 无法无人值守运行 |
| 通过 Gateway 远程控制 | 无 screenX/Y 坐标修复 |
| Playwright 内置 OOPIF/弹窗/下载 | 3 进程 Playwright 架构（更高延迟） |
