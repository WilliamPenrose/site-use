...
| 能力 | PRB | site-use | 建议 |
|------|-----|----------|------|
| MouseEvent `screenX/screenY` 坐标修复 | ✅ `evaluateOnNewDocument` | ❌ 未实现 | ✅ 推荐 M3 实现 |
| `Emulation.setFocusEmulationEnabled` | ❌ 未实现 | ❌ 未实现（已规划） | ✅ 推荐 M1 实现 |
...
**建议：推荐 M3 实现。** 实现成本极低（一次 `evaluateOnNewDocument` 调用），但需要与鼠标轨迹模拟（同为 M3）配合才有完整效果。

### L5 — 行为模拟

| 能力 | PRB | site-use | 建议 |
|------|-----|----------|------|
| 贝塞尔曲线鼠标轨迹 | ✅ ghost-cursor | ❌ 未实现 | ✅ 推荐 M3 |
| `realClick()` API | ✅ | ❌ | ✅ 推荐 M3 |
| 操作节流（随机延迟） | ❌ | ✅ throttle.ts | ✅ site-use 已有 |

**分析：** PRB 集成了 `ghost-cursor` 提供贝塞尔曲线鼠标移动。Puppeteer 原生 `page.click()` 直接在目标坐标触发 click，没有 `mousemove` 事件序列——零 `mousemove` 是最强的自动化信号之一。

site-use 的 `throttle.ts` 解决了操作频率问题，但没有解决鼠标轨迹问题。

**建议：M3 实现鼠标轨迹模拟**（已在 site-use TODO 中）。可直接集成 `ghost-cursor` 或自研贝塞尔曲线算法。
...
| 优先级 | 差距 | PRB 方案 | site-use 建议方案 | 里程碑 |
|--------|------|---------|-------------------|--------|
| **高** | `document.hasFocus()` 检测 | 未覆盖 | `Emulation.setFocusEmulationEnabled`（DP-3） | M1 |
| **高** | Puppeteer 默认参数指纹 | connect 架构绕过 | 扩大 `ignoreDefaultArgs` 排除列表 | M1 |
| **中** | CDP 协议泄漏 | rebrowser-puppeteer-core | 评估替换为 rebrowser 或自研修补 | M3 |
| **中** | MouseEvent 坐标不一致 | `evaluateOnNewDocument` | 同方案 | M3 |
| **中** | 鼠标轨迹缺失 | ghost-cursor | 集成 ghost-cursor 或自研 | M3 |
