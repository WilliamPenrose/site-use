...
**状态：已实现。** 自研贝塞尔曲线算法（`generateBezierPath()`），随机控制点模拟手部运动。`PuppeteerBackend.click()` 默认走增强路径，生成 20-60+ mousemove 事件。另外增加了 `applyJitter()` ±3px 坐标抖动、`checkOcclusion()` 遮挡检测、`waitForElementStable()` 动画等待。所有增强可通过环境变量单独关闭。
