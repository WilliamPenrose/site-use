...
M2："完整的 Twitter 自动化"（在 M1 基础上增量）
    详见 m2/00-full-twitter-automation.md
...
| 搜索推文 | M6 存储层 ingest | `searchTweets(query, opts?)` workflow — Twitter API Search，结果自动落盘到 M6 本地库 |
...
对应 PRD 全部 4 个场景。用户可以说"帮我关注 Vitalik"、"看看 @elonmusk 最近发了什么"、"搜一下最近关于 AI agent 的推文"。完整的 Twitter 自动化工具。`twitter_search_tweets` 的搜索结果自动落盘到 M6 本地库，实现"先搜本地、再搜全网"的分层检索。
...
| 搜索 | twitter_search_user, twitter_search_tweets | search_feeds（同上，含搜索参数） |
...
> 设计讨论阶段，详见 [m6/00-raw-data-archive.md](m6/00-raw-data-archive.md)

site-use 是原始数据的忠实记录者，不是智能知识库。与上层 agent（OpenClaw 等）形成分级存储：下层零成本无损保存原始数据，上层按需 LLM 提炼。

核心约束：零外部服务、零 API 调用。技术栈：SQLite（better-sqlite3）+ sqlite-vec（向量搜索）+ FTS5（全文搜索）+ transformers.js（本地 ONNX embedding）。检索管线参考 OpenClaw（向量+BM25+混合融合+MMR+时间衰减），新增结构化筛选（按作者/时间/标签）。推文天然是结构化短文本，不需要分块，不需要 LLM，tweet.id 天然去重。
...
浏览网页时通过 CDP 网络拦截捕获视频/图片资源。很多网站允许播放但阻止下载——CDP 可以在浏览器加载时截获已解密的媒体流。这是通用的浏览器能力，不限于特定站点。

**关键考量：**
- 在 primitives 层拦截 `image/*` 和 `video/*` 响应
- HLS/DASH 流需要分片重组
- 存储：本地文件系统，不放 SQLite（二进制大文件）
- 与搜索/知识库独立——不同的数据生命周期

### 从 raw_json 重建辅助表

扩展 `rebuild` 命令，从 `items.raw_json` 重新派生辅助表（hashtags、mentions、links）。当前阻塞点：raw_json 存的是处理后的 `Tweet` 对象，而非原始 GraphQL entities。需要先决定 raw_json 应该保留什么——可能是一个轻量的中间层（如仅保留 `entities` + `extended_entities`），在存储成本和重建能力之间取得平衡。
...
- 2026-03-23：新增 M6 个人知识存储里程碑
  - 调研 agent-friendly 知识存储全景（Mem0/Graphiti/Letta/Cognee/Khoj/Hindsight）
  - 分析本地 OpenClaw（SQLite+FTS5+sqlite-vec）和 GitNexus（LadybugDB 图数据库）的存储架构
  - 决策：不用 Mem0 的 LLM 管线（推文已是结构化数据，不需要 LLM 提取）
  - 决策：零外部服务（不依赖 Neo4j/Docker/Ollama），全部嵌入式库
  - 决策：本地 embedding（transformers.js + ONNX），支持模型热切换
  - 技术栈：SQLite + sqlite-vec + FTS5 + transformers.js + Kuzu（可选）
