...
describe('action workflow pipeline', () => {
  it('collection workflow result with items does not trigger side effects', async () => {
  ...
  it('action workflow with dailyLimit throws when limit exceeded', async () => {
    ...
    vi.doMock('../../src/action-log/index.js', () => ({
      getDailyActionCount: vi.fn().mockReturnValue(50),
      logAction: vi.fn(),
      openActionLogDb: () => ({ close: vi.fn() }),
    }));
    ...
    vi.doUnmock('../../src/action-log/index.js');
